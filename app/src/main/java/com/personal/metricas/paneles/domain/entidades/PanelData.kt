...
data class PanelData(
        ...
        var filaSeleccionada: Fila? = null,
) {
        ...
        fun fromPanelUI(
            ...
            when (panelUI.tipoPanel) {
                ...
                TiposPanel.PANEL_END_POINT -> {
                    ...
                    var listaParametrosReemplazados: MutableList<Parametro> =
                            mutableListOf<Parametro>()
                    endPoint.parametros.ps.forEach { parametro ->
                        val s: String =
                                Parametros.reemplazar(
                                        parametro.valor,
                                        endPoint.parametros,
                                        parametrosOrigenDatos
                                )
                ...
                TiposPanel.PANEL_CONECTOR -> {
                    val sql =
                            parametrosOrigenDatos.addFiltrosOrganizacionLectora(
                                    pui.kpi.sql,
                                    pui.configuracion.filtroOrganizacion,
                                    pui.configuracion.filtroLectora
                            )
                    tabla =
                            ResultadoSQL.fromSqlToTabla(
                                    sql = sql,
            ...
            val titulo: String =
                    Parametros.reemplazar(pui.titulo, endPoint.parametros, parametrosOrigenDatos)
    ...
    fun ordenarElementos() =
            valoresTabla.dameElementosOrdenados(campoOrdenacionTabla = panelConfiguracion.columnaY)
    ...
    fun aplicarCondicionesFilas(): List<Fila> {
        ...
        // establecemos los colores que va a tener cada elmento (teninedo en cuenta que deben al
        // menos tener todos los coles declarados.
        val colores = EsquemaColores().getColores(panelConfiguracion.colores)
        val totalColores: Int = colores.size
        valoresTabla.filas =
                valoresTabla.filas.mapIndexed { index, f ->
        ...
        if (panelConfiguracion.condiciones.isNotEmpty()) {
            ...
            valoresTabla.filas =
                    valoresTabla.filas.mapIndexed { indice, fila ->
                        ...
                        panelConfiguracion.condiciones.forEach { condicion ->
                            ...
                            if (valor.isNotEmpty()) {
                                ...
                                if ((condicion.condicionCelda > 0 &&
                                                condicion.predicado.isEmpty()) ||
                                                (resultado is Boolean && resultado)
                                ) {
                                    color =
                                            EsquemaColores()
        ...
        if (panelConfiguracion.condicionesCeldas.isNotEmpty()) {
            // Inicializamos las herramientas fuera del bucle principal para no recrearlas en cada
            // iteración.
            val jexl = JexlBuilder().create()
            ...
            val expresionesCompiladas =
                    panelConfiguracion.condicionesCeldas.associateWith { condicion ->
                        try {
            ...
            valoresTabla.filas =
                    valoresTabla.filas.map { fila ->
                        // Convertimos la lista de celdas a una lista mutable para poder modificarla
                        // eficientemente.
                        ...
                        panelConfiguracion.condicionesCeldas.forEach { condicionCelda ->
                            try {
                                ...
                                val expresion =
                                        expresionesCompiladas[condicionCelda]
                                                ?: return@forEach // Salta a la siguiente condición
                                ...
                                if ((condicionCelda.condicionCelda > 0 &&
                                                condicionCelda.predicado.isEmpty()) ||
                                                (resultadoCondicion is Boolean &&
                                                        resultadoCondicion)
                                ) {
                                    val columna =
                                            fila.dameColumnaVacia(condicionCelda.columna.nombre)
                                    ...
                                    if (columna != null) {
                                        ...
                                        val colorCondicion =
                                                esquemaColoresCondiciones.colores[
                                                        condicionCelda.color]
                                        val celdaOriginal = celdasModificables[posicionEvaluar]

                                        // Creamos la nueva celda y la reemplazamos directamente en
                                        // la lista mutable.
                                        celdasModificables[posicionEvaluar] =
                                                if (condicionCelda.condicionCelda > 0) {
                                                    celdaOriginal.copy(
                                                            colorCelda = colorCondicion,
                        ...
                        // Si se aplicó alguna modificación, creamos una nueva fila con las celdas
                        // actualizadas.
                        // Si no, devolvemos la fila original para evitar la creación de objetos
                        // innecesarios.
    ...
    private fun gestionAlarma(condicion: Condiciones, fila: Fila) {
        if (condicion.alarma.activa) {
            val id =
                    if3(
                            condicion.alarma.unica,
                            "00${panel.id}00${panel.kpi.id}00${condicion.id}",
                            System.currentTimeMillis().toString()
                    )
            val alm =
                    condicion.alarma.copy(
                            id = id,
                            color = condicion.color,
                            titulo = "${getAppName(App.context)} | ${condicion.alarma.titulo}",
                            texto = notificacionesManager.dameTexto(condicion.alarma.texto, fila)
                    )
            if (listaAlarmas.filter { it.id.equals(id) }.size == 0) {
                listaAlarmas.add(alm)
                notificacionesManager.showNotificacion(context = App.context, alarma = alm)
            }
        }
    }
}
