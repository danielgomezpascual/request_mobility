...
class MyLog(
	private val configuracion: ConfiguracionMyLog,
	private val output: IOutputLog,
) {
	...
	fun lista(
		titulo: String = "Lista de elementos", l: List<Any>, completa: Boolean = false,
		numElementos: Int = 10,
	) = lista(l, titulo, completa, numElementos)


	fun lista(
		l: List<Any>, titulo: String = "Lista de elementos", completa: Boolean = false,
		numElementos: Int = 10,
	) {
		enter()
		val posicionTrazaAnterior = configuracion.trazaInformacionStackTrace;

		configuracion.trazaInformacionStackTrace = 10

		/*(1..10).forEach {
			configuracion.trazaInformacionStackTrace = it
			importante("$it - $titulo")
		}*/
		importante("$titulo")
		configuracion.trazaInformacionStackTrace = configuracion.trazaInformacionListasStackTrace
		if (l.isEmpty()) {
			d("Vacio")
			linea()
			enter()
		...
		configuracion.trazaInformacionStackTrace = posicionTrazaAnterior
