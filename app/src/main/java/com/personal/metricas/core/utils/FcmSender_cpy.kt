...
object FcmSender_cpy {

	// --- CONFIGURACIÓN: PEGA AQUÍ LOS DATOS DE TU JSON ---
	// NOTA: El private_key debe copiarse tal cual aparece en el JSON, incluyendo los \n
	private const val CLIENT_EMAIL = "firebase-adminsdk-ywqlm@development-apps-3e540.iam.gserviceaccount.com"
	...
	private const val PROJECT_ID = "development-apps-3e540"
	 ...
	 */
	suspend fun sendNotification(deviceToken: String, title: String, bodyText: String): String {
		return withContext(Dispatchers.IO) {
			try {
				...
				App.log.d("TOken reminte $tokenPropioFirebase")
				...
				val jsonBody = JSONObject().apply {
					put("message", JSONObject().apply {
						...
						put("data", JSONObject().apply {
							put("mob_request_id", "825945")
							put("accion", "MOB_REQUEST_ID")
							put("remitente", tokenPropioFirebase)
						})
					})
				}
	...
	private fun getAccessToken(): String {
		...
		val response = client.newCall(request).execute()
		if (!response.isSuccessful) throw Exception("Error obteniendo token: ${response.body?.string()}")
