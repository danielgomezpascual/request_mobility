...
class GenerateExcel {
    ...
    fun generate(titulo: String = "Sin titulo", filas: List<Fila>, fichero: File) {
        ...
        // TITULO
        // 1. Crea la fila y la celda donde comenzará la combinación (la de la esquina superior
        // izquierda)
        ...
        val region =
                CellRangeAddress(
                        ...
                        desplazamientoFila, // última fila (la misma porque es una combinación
                        // horizontal)
                        desplazamientoCelda, // primera columna
                        desplazamientoCelda +
                                filas.first().celdas.size // última columna (A=0, B=1, C=2)
                )
        ...
        val estiloCabecera = workbook.createCellStyle().apply { setFont(font) }
        ...
        filas.forEachIndexed { indexFila, fila ->
            ...
            fila.celdas.dropLast(1).forEachIndexed { indexCelda, celda ->
                ...
                cell.applyBackgroundColorFromHex(
                        ColorManager.toPastelColor(ColorManager.toHexCode(fila.color), 0.9f)
                )
            }
        }

        // AJUSTE DE ANCHO DE COLUMNAS
        // Calculamos el rango de filas que pertenecen a la tabla (cabecera + datos)
        val primeraFilaTabla = (desplazamientoFila - 1) // Donde está la cabecera
        val ultimaFilaTabla = (desplazamientoFila + filas.size - 1)

        filas.first().celdas.dropLast(1).forEachIndexed { index, _ ->
            sheet.autoSizeColumnAndroid(
                    columnIndex = desplazamientoCelda + index,
                    startRow = primeraFilaTabla,
                    endRow = ultimaFilaTabla,
                    maxContentLength =
                            100 // Un valor razonable para evitar celdas absurdamente anchas
            )
        }
