...
object SQL {
    ...
    val INFO_TRANSACCIONES_HOY: String =
			...
			SELECT
				...
				USUARIO_LECTORA 
			FROM
				TRX_TIME  T LEFT JOIN ESTADOS_TRANSACCIONES ET ON T.REQ_STATUS = ET.STATUS_CODE 
...
ORDER BY
    ...
    val LECTORAS_TRANSACCIONES =
		 ...
		 SELECT DISTINCT  LECTORA_FISICA_ID ,
		  ORGANIZATION_CODE,
		   ORGANIZATION_NAME 
		   FROM TRX_TIME ORDER BY LECTORA_FISICA_ID ASC
...
""".trimIndent()
    ...
    val ERRORES_DISPOSITIVO_FISICO =
            """ SELECT
			LECTORA_FISICA_ID,
...
ORDER BY
    ...
    T1.REQ_STATUS,
    T3.ESTADO AS ESTADO_DESCRIPCION,
    T1.total_por_estado,
...
ORDER BY
    ...
    ROUND((CAST(COUNT(*) AS REAL) * 100.0) / (SELECT COUNT(*) FROM TRX_TIME T2 WHERE T2.PROGRAM_VERSION = T1.PROGRAM_VERSION), 2) AS PVERS
FROM
    TRX_TIME T1
WHERE
